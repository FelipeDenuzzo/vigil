// assessment-v2/runVisualSearchV2.ts
// Orquestrador do avaliador v2.
// Recebe o SessionLog existente, converte para V2RoundInput e retorna VisualSearchV2Result.
// NÃO substitui nem interfere com o avaliador v1.
...
function toV2Rounds(log: { rounds?: any[] }): V2RoundInput[] {
  return (log.rounds ?? []).map((round: any, idx: number) => {
    const hits =
      round.hits ??
      (round.clicks?.filter(
        (c: any) => c.action === "mark" && c.isTarget
      ).length ?? 0);
    const errors =
      round.errors ??
      (round.clicks?.filter(
...
// ─── Eagle Score proxy (comparação direta com v1) ────────────────────────────
function calcEagle(rounds: V2RoundInput[]): { score: number; label: string } {
  ...
  const omission =
    totalTargets > 0 ? totalMissed / totalTargets : 0;
  const commission =
    totalHits + totalErrors > 0
      ? totalErrors / (totalHits + totalErrors)
      : 0;
  const score = Math.max(
    0,
    Math.min(100, Math.round(100 - omission * 45 - commission * 35))
  );
  const label =
...
function calcConjunctionBreak(rounds: V2RoundInput[]): ConjunctionBreakResult {
  ...
  return {
    ...
    shortDescription: detected
      ? `Queda abrupta a partir do nível ${collapseAtLevel}. Desempenho caiu de ${Math.round(earlyMean * 100)}% para ${Math.round(laterMean * 100)}%.`
...
export function runVisualSearchV2(
  log: { sessionId: string; gameId: string; rounds?: any[] }
): VisualSearchV2Result {
