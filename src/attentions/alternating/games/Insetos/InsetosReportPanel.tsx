...
// Painel de relatório pós-sessão Insetos — espelha ColorShapeReportPanel.
...
const s = {
  ...
  header: { padding: '16px 16px 0' } as const,
  title: { fontSize: 16, fontWeight: 700, color: '#e8e9f0', marginBottom: 12 } as const,
  tabRow: {
    display: 'grid',
  ...
  body: { padding: 16, display: 'grid', gap: 12 } as const,
  ...
  recommendation: {
    ...
    borderRadius: 10, textAlign: 'center' as const, marginTop: 8,
  },
  analysisBlock: {
  ...
  listItem: {
    display: 'flex', gap: 8, alignItems: 'flex-start' as const,
  ...
  divider: { height: 1, background: 'rgba(255,255,255,0.06)', margin: '2px 0' } as const,
...
export function InsetosReportPanel({ report }: { report: EvaluationReport }) {
  ...
  return (
    <div className="result-card" style={s.wrapper}>
      ...
      <div style={s.body}>
        {tab === 'ludic' && (
          <>
            <ReguaLudica score={report.score} level={report.level} />
            {recommendation && (
        ...
        {tab === 'analysis' && (
          <>
            {clinicalNote && (
              <div style={s.analysisBlock}>{clinicalNote}</div>
            )}
